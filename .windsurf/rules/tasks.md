...
- [ ] Prace.cz: ПРОВЕРЕНО (2026-08-03) — другая платформа (Next.js/RSC, не движок jobs.cz), но контент ~100% дублирует jobs.cz (одинаковые вакансии/работодатели, вероятна общая синдикация в рамках холдинга). Добавление дало бы задвоенные уведомления без пользы. Не добавлять.
